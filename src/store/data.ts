...
export const loadMusicData = async () => {
  try {
    ...
    const imgModules = import.meta.glob("@/assets/images/music/*.webp", {
      eager: true,
    });
    // 生成音乐列表
    musicList.value = await Promise.all(
      Object.entries(mp3Modules).map(async ([mp3Path, mp3Url]) => {
        ...
        const musicLogoModule = (imgModules[imgPath] as {
          default: string;
        }) || {
          default: "",
        };
